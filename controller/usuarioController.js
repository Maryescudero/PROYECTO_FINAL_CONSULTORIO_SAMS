...
exports.registrarUsuario = async (req, res) => {
  ...
  // Verificar que  sea administrador o profesional
  if (funcion !== 'administrador' && funcion !== 'profesional') {
    return res.send('La función debe ser "administrador" o "profesional".');
  }
  ...
  try {
    ...
    const query = 'INSERT INTO usuario (nombre, apellido, funcion, usuario, password, estado) VALUES (?, ?, ?, ?, ?, ?)';
    await conexion.query(query, [nombre, apellido, funcion, usuario, hashedPassword, 1]); 
...
exports.procesarLogin = async (req, res) => {
  ...
  try {
    ...
    // Verificar  contraseña coincide y el estado 
    if (!match) {
      await conexion.end();
      return res.render('usuario/login', { errorMessage: 'Usuario o contraseña incorrectos.' });
    }
    ...
    if (user.estado === 0) {
      await conexion.end();
      return res.render('usuario/login', { errorMessage: 'El usuario está inactivo.' });
    }

    // guarda el usuario en la sesiom
    req.session.loggedin = true;
    req.session.usuario = user;
